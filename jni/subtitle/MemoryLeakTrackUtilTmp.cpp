 ...
 */
#define LOG_NDEBUG 0
#define LOG_TAG "MemoryLeakTrackUtilTmp"
...
//#if defined(__arm__)
    ...
    struct MyString8
    {
            static const size_t MAX_SIZE = 16 * 1024 * 1024;    //256 * 1024;
    ...
    void dumpMemoryAddresses(int fd)
    {
        ...
        ALOGE("[dumpMemoryAddresses]fd:%d\n", fd);
        ...
        if (info)
        {
            ...
            {
                AllocEntry *e = &entries[i];
                snprintf(buffer, SIZE, "size %8i, dup %4i, ",
                         e->size, e->dups);
                result.append(buffer);
                for (size_t ct = 0;
                        (ct < backtraceSize) && e->backtrace[ct];
                        ct++)
                {
                    if (ct)
                    {
